...
local Window = Rayfield:CreateWindow({
   ...
   KeySystem = true,
   KeySettings = {
      Title = "Projectsion Key System",
      Subtitle = "Enter your key",
      Note = "Here the key projectsion1445",
      FileName = "projectsionKey",
      SaveKey = false,
...
-- // 5. FUNGSI WEBHOOK (VOIDLINE EMBED AUTHOR VERSION)
local function sendWebhook(income, target)
    ...
    -- DISESUAIKAN: Pakai StatsFolder.Uang (Bus Explorer)
    ...
    local embed = {
        ...
        ["title"] = "Bus Route Completed", -- Diubah dikit biar nyambung
        ["color"] = 0xFFFFFF,

        ["fields"] = {
            {["name"] = "Username", ["value"] = LP.Name, ["inline"] = false},
            {["name"] = "Cycle Income", ["value"] = formatRP(income), ["inline"] = false},
            {["name"] = "Target", ["value"] = "Cirebon Route", ["inline"] = false}, -- Info rute lu
...
-- // 6. MONEY MONITOR (Setiap 2.15 Menit / 135 Detik)
        ...
        while _G.AutoFull do
            ...
            if not jobStarted then
                ...
                if bus and bus:FindFirstChild("DriveSeat") then
                    ...
                    SetStatus("get job...")
                    Remotes:WaitForChild("StartBusJob"):InvokeServer("Baranangsiang_ke_Cirebon2")
                    task.wait(1)
