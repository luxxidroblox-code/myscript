...
-- ── Delta-safe index-call helper ──────────────────────────────────────────────
-- routes obj:Method() through indexing, bypasses __namecall entirely
local function sc(obj, method, ...)
    return obj[method](obj, ...)
end
-- ─────────────────────────────────────────────────────────────────────────────

-- ── namecallInstance bypass: hook __namecall BEFORE Adonis loads ──────────────
...
do
    ...
    _hookmetamethod(game, "__namecall", _newcclosure(function(self, ...)
        if not _checkcaller() then
            -- Adonis reads caller source via debug.info on its detection thread.
            -- Yielding suspends that coroutine so debug.info returns blank source.
            local co = coroutine.running()
            if co then
                task.defer(function() coroutine.resume(co) end)
                coroutine.yield()
            end
        end
        return orig(self, ...)
...
local HttpService       = sc(game, "GetService", "HttpService")
local ReplicatedStorage = sc(game, "GetService", "ReplicatedStorage")
local TweenService      = sc(game, "GetService", "TweenService")
local Players           = sc(game, "GetService", "Players")
local LP                = Players.LocalPlayer
local RunService        = sc(game, "GetService", "RunService")
local PlayerData        = LP:WaitForChild("PlayerData")
local UserInputService  = sc(game, "GetService", "UserInputService")
local Workspace         = sc(game, "GetService", "Workspace")
local TeleportService   = sc(game, "GetService", "TeleportService")

-- ── Wrap all executor globals in newcclosure ──────────────────────────────────
local _fireproximityprompt = _newcclosure(function(prompt, ...)
    return fireproximityprompt(prompt, ...)
end)

local _firesignal = firesignal and _newcclosure(function(sig, ...)
    return firesignal(sig, ...)
end) or nil

local _hookfunction = hookfunction and _newcclosure(function(a, b)
    return hookfunction(a, b)
end) or nil

local _getgc = getgc and _newcclosure(function(...)
    return getgc(...)
end) or nil

-- TweenService:Create routed through sc() — namecall origin sanitized
local _TweenCreate = _newcclosure(function(info, target, props)
    return sc(TweenService, "Create", target, info, props)
end)
-- ─────────────────────────────────────────────────────────────────────────────
...
local function CreateStabilizer(root)
    ...
    pos.MaxForce      = Vector3.new(0, 1e6, 0)
    pos.P             = 1e5
    pos.D             = 500
...
local function DestroyStabilizer(gyro, pos)
    if gyro and gyro.Parent then gyro:Destroy() end
    if pos  and pos.Parent  then pos:Destroy()  end
end
-- ─────────────────────────────────────────────────────────────────────────────
...
local function RejoinServer()
    ...
    pcall(function()
        if #Players:GetPlayers() <= 1 then
            sc(TeleportService, "Teleport", game.PlaceId, LP)
        else
            sc(TeleportService, "TeleportToPlaceInstance", game.PlaceId, game.JobId, LP)
        end
    end)
end
...
local function SwitchToCourier()
    local JobEvents = sc(ReplicatedStorage, "WaitForChild", "JobEvents")
    local Event     = sc(JobEvents, "WaitForChild", "TeamChangeRequest")
    if not LP.Team or LP.Team.Name ~= "Courier" then
        sc(Event, "FireServer", "Courier", 11378976, 1, 0, "MainMenu")
        task.wait(2)
    end
end
...
local function SwitchToPolice()
    ...
    if not LP.Team or LP.Team.Name ~= "Police" then
        sc(Event, "FireServer", "Police", 0, 0, 1428858969, "MainMenu")
        task.wait(2)
    end
end
...
local function Tween(targetCFrame)
    ...
    local gyro, pos = CreateStabilizer(Root)
    ...
    local tween = _TweenCreate(info, Root, {CFrame = targetCFrame})
    tween:Play()
    ...
    if bp then
        for _, item in pairs(bp:GetChildren()) do
            if item:IsA("Tool") and item.Name:lower() == "box" then
                sc(Char.Humanoid, "EquipTool", item)
                return true
            end
        end
    end
...
task.spawn(function()
    local VirtualUser = sc(game, "GetService", "VirtualUser")
    sc(game, "GetService", "Players").LocalPlayer.Idled:Connect(function()
        sc(VirtualUser, "CaptureController")
        sc(VirtualUser, "ClickButton2", Vector2.new())
    end)
end)
...
local function sendWebhook(income, target)
    ...
    local payload = sc(HttpService, "JSONEncode", {["username"] = "Projectsion Reports", ["embeds"] = {embed}})
...
local function BypassTP(targetCF)
    ...
    local info  = TweenInfo.new(duration, Enum.EasingStyle.Linear)
    local tween = _TweenCreate(info, Root, {CFrame = targetCF})
    tween:Play()
    tween.Completed:Wait()
...
local function ExecuteStartSequence()
    ...
    if LP.Team and LP.Team.Name ~= "Barista" and tr then
        sc(tr, "FireServer", "Barista", 11378976, 0, 0, "Detector")
        task.wait(2.5)
    end
            ...
            if Hum and Root and Hum.Health > 0 then
                ...
                if not AutoEquipBox() then
                    ...
                    if (os.clock() - WaktuKosong) >= 240 then
                        local JobEvents = sc(ReplicatedStorage, "WaitForChild", "JobEvents")
                        local tcr       = sc(JobEvents, "WaitForChild", "TeamChangeRequest")
                        sc(tcr, "FireServer", "Civilian", 0, 0, 0, "MainMenu")
...
-- ── getgc hook: tightened to 3s window ───────────────────────────────────────
...
local idledConn = LP.Idled:Connect(function()
    pcall(function()
        local VirtualUser = sc(game, "GetService", "VirtualUser")
        sc(VirtualUser, "CaptureController")
        sc(VirtualUser, "Button2Down", Vector2.new(0, 0), workspace.CurrentCamera.CFrame)
        task.wait(0.2)
        sc(VirtualUser, "Button2Up", Vector2.new(0, 0), workspace.CurrentCamera.CFrame)
    end)
end)
...
local function UnanchorAll()
    ...
    if Humanoid then
        pcall(function()
            ...
            Humanoid.Sit = false
            sc(Humanoid, "SetStateEnabled", Enum.HumanoidStateType.Seated, false)
            task.wait(0.15)
            Humanoid.Jump = true
...
local function LerpTeleport(HRP, destCFrame, duration)
    ...
    local tween = _TweenCreate(TweenInfo.new(duration, Enum.EasingStyle.Linear), HRP, {CFrame = destCFrame})
    tween:Play()
    tween.Completed:Wait()
...
local function SafePoliceTeleport(targetCFrame, bypassChecks, preventUnsit, skipSit)
    ...
    pcall(function()
        ...
        if isDriving then
            ...
            if not skipSit then
                local gyro, pos = CreateStabilizer(HRP)
                task.wait(0.2)

                local distance2 = (destCFrame.Position - HRP.Position).Magnitude
                ...
                task.wait(math.random(postWait.min, postWait.max))
        ...
        if duration > 0 then
            if isDriving then
                ...
                local tween = _TweenCreate(TweenInfo.new(duration, Enum.EasingStyle.Linear), mainPart, {CFrame = destCFrame})
...
local function FirePrompt(prompt, targetPart)
    ...
    for attempt = 1, 3 do
        ...
        if not triggered and prompt and prompt.Parent and prompt.Enabled then
            pcall(function()
                sc(prompt, "InputHoldBegin")
                task.wait(prompt.HoldDuration + 0.3)
                sc(prompt, "InputHoldEnd")
            end)
            task.wait(0.5)
            if not prompt or not prompt.Parent or not prompt.Enabled then triggered = true; break end
        end
    ...
    pcall(function()
        ...
        if Character then
            ...
            if HRP then
                ...
                UnanchorAll()
                task.wait(1.5)
                local PoliceJob = sc(Workspace, "WaitForChild", "PoliceJob", 10)
                local Start     = PoliceJob and sc(PoliceJob, "WaitForChild", "Start", 10)
                local TOMBOL    = Start and sc(Start, "WaitForChild", "ProximityPrompt", 10)
                if TOMBOL then
                    task.wait(1)
...
local function PlaceConeAtZone(zonePart)
    ...
    if tool then
        ...
        if remote then pcall(function() sc(remote, "FireServer", targetPos, HRP.CFrame) end) end
        task.wait(0.3)
        pcall(function()
            local PoliceAssets = sc(ReplicatedStorage, "WaitForChild", "PoliceAssets", 10)
            local Event        = PoliceAssets and sc(PoliceAssets, "WaitForChild", "PoliceEvent", 10)
        ...
        if not success then
            pcall(function() sc(tool, "Activate") end)
            task.wait(1)
...
local function NeutralizeSuspect(missionModel, suspect)
    ...
    pcall(function()
        local Character = LP.Character
        local Humanoid  = Character and Character:FindFirstChildOfClass("Humanoid")
        if Humanoid then sc(Humanoid, "UnequipTools") end
    end)
end
...
HomeTab:CreateButton({
    Name = "Version 1.5 - Delta namecall Fix",
    Callback = function()
        Rayfield:Notify({
            Title    = "Projectsion",
            Content  = "+ Real coroutine yield in __namecall hook\n+ sc() routes all critical calls through index-call\n+ FireServer / EquipTool / Activate / SetStateEnabled all patched",
            Duration = 5
        })
    end
})
...
Rayfield:Notify({
    Title    = "Projectsion",
    Content  = "Loaded Successfully - v1.5 Delta namecall Fix Active",
    Duration = 5
})
