...
-- ── BodyGyro Stabilizer (replaces all Hum.Sit bypass) ───────────────────────
    ...
    pcall(function()
        ...
        if isDriving then
            ...
            if not skipSit then
                ...
                task.wait(0.2)
                -- gyro/pos held alive across the tween, destroyed after
...
HomeTab:CreateButton({
    Name = "Version 1.3 - BodyGyro Stabilizer",
    Callback = function()
        Rayfield:Notify({
            Title = "Projectsion",
            Content = "+ Sit bypass replaced with BodyGyro+BodyPosition stabilizer\n+ HumanoidStateType.Seated no longer touched during tween\n+ stabilizer destroyed immediately on arrival, zero persistent footprint",
            Duration = 5
        })
    end
})
...
Rayfield:Notify({
    Title = "Projectsion",
    Content = "Loaded Successfully - v1.3 BodyGyro Bypass Active",
    Duration = 5
})
