--[[
    ...
    Adonis AC Bypass v3 + Fixed workspace.General
]]
...
-- ============================================================
-- PHASE 1: Purge connections BEFORE AC initializes
-- ============================================================
local function purgeConnections(service, filter)
    local conns = getconnections(service)
...
purgeConnections(LogService.MessageOut)
purgeConnections(ScriptContext.Error)

Players.LocalPlayer.CharacterAdded:Connect(function()
    task.delay(0.5, function() purgeConnections(LogService.MessageOut) end)
    task.delay(2.0, function() purgeConnections(LogService.MessageOut) end)
end)

-- ============================================================
-- PHASE 2: loadstring canary — environment patch
-- ============================================================
local loadstringEnv = getfenv(loadstring)
local realLoadstring = loadstringEnv.loadstring or loadstring

loadstringEnv.loadstring = function(src, ...)
...
-- ============================================================
-- PHASE 4: RobloxLocked — metatable patch
-- ============================================================
local StringValueMeta = getrawmetatable(Instance.new("StringValue"))
local oldNewIndex = StringValueMeta.__newindex

setreadonly(StringValueMeta, false)
StringValueMeta.__newindex = function(self, key, value)
    if key == "RobloxLocked" then
        error("Property RobloxLocked is not accessible", 2)
    end
    return oldNewIndex(self, key, value)
end
setreadonly(StringValueMeta, true)

-- ============================================================
-- PHASE 5: GetRealPhysicsFPS — environment override
-- ============================================================
local workspaceEnv = getfenv(workspace.GetRealPhysicsFPS)
workspaceEnv.GetRealPhysicsFPS = function(self)
    local real = rawget(workspaceEnv, "_GetRealPhysicsFPS")
    local fps = real and real(self) or 60
    return math.min(fps, 59)
end

-- ============================================================
-- PHASE 6: Humanoid state — SetStateEnabled patch
-- ============================================================
local function disableNoClipState(char)
...
Players.LocalPlayer.CharacterAdded:Connect(disableNoClipState)
if Players.LocalPlayer.Character then
    task.spawn(disableNoClipState, Players.LocalPlayer.Character)
end

-- ============================================================
-- PHASE 7: GetLogHistory — table reference swap (NO hook)
-- ============================================================
local function patchLogHistory()
    local logService = game:GetService("LogService")
    local realHistory = {}
    
    local getFunc = logService.GetLogHistory
    if getFunc then
        local info = debug.getinfo(getFunc)
        for i = 1, info.nupvalues or 0 do
            local name, value = debug.getupvalue(getFunc, i)
            if type(value) == "table" and name and (name:find("history") or name:find("log")) then
                realHistory = value
                break
            end
        end
    ...
    local shadow = {}
    local patterns = {"hookfunction", "hookmetamethod", "getrawmetatable", 
                      "identifyexecutor", "getconnections", "setreadonly", 
                      "newcclosure", "gethui", "getupvalue", "setupvalue"}
    
    setmetatable(shadow, {
        __index = function(t, k)
            local val = realHistory[k]
        ...
        __len = function()
            local count = 0
            for k, v in pairs(realHistory) do
                if type(v) == "string" then
                    local dirty = false
                    for _, pat in ipairs(patterns) do
                        if v:lower():find(pat, 1, true) then dirty = true break end
                    end
                    if not dirty then count = count + 1 end
                else
                    count = count + 1
                end
            end
        ...
        __pairs = function()
            return function(t, k)
                local nextKey, nextVal = next(realHistory, k)
                while nextKey do
                    if type(nextVal) == "string" then
                        local dirty = false
                        for _, pat in ipairs(patterns) do
                            if nextVal:lower():find(pat, 1, true) then dirty = true break end
                        end
                        if not dirty then
                    ...
                    nextKey, nextVal = next(realHistory, nextKey)
                end
                return nil
            end
        end
    ...
    if getFunc then
        local info = debug.getinfo(getFunc)
        for i = 1, info.nupvalues or 0 do
            local name, _ = debug.getupvalue(getFunc, i)
            if name and (name:find("history") or name:find("log")) then
                debug.setupvalue(getFunc, i, shadow)
                break
            end
        end
    end
...
task.delay(0.2, patchLogHistory)

-- ============================================================
-- PHASE 8: Disallowed services — findservice patch
-- ============================================================
local findServiceEnv = getfenv(game.FindService)
local realFindService = findServiceEnv.FindService or game.FindService

findServiceEnv.FindService = function(self, name)
    local blocked = {"VirtualUser", "VirtualInputManager", "UGCValidationService"}
    for _, b in ipairs(blocked) do
        if name == b then return nil end
    end
...
task.delay(4, function()
    pcall(function()
        for _, v in ipairs(getloadedmodules()) do
            if tostring(v.Name):find("Anti") or tostring(v.Name):find("Adonis") then
                local env = getsenv(v)
                if env and env.Anti and env.Anti.Detected then
                    env.Anti.Detected = function() return end
                end
            end
        end
    end)
...
local function main()
    ...
    local function newFarming()
        ...
        while autofarmstart do
            ...
            -- FIX: workspace.General existence check with retry
            local waypoint = nil
            local retries = 0
            while retries < 30 and not waypoint and autofarmstart do
                local general = workspace:FindFirstChild("General")
                if general then
                    waypoint = general:FindFirstChild("Waypoint")
                end
...
print("[+] Projectsion loaded. AC bypass active.")