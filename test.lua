...
local function checkForAdmin(targetPlayer)
    if not AntiAdminEnabled or targetPlayer == Player then
        return
    end
    ...
    pcall(function()
        if targetPlayer:GetRankInGroup(GAME_GROUP_ID) >= MIN_STAFF_RANK then
            if vStatus then
                vStatus.Text = "Admin detected - leaving"
            end
            task.wait(0.5)
...
local function saveConfig()
    ...
    writefile(CFG_FILE, '{"speed":' .. SPEED
        ...
        .. ',"webhookUrl":"' .. webhookUrl:gsub('"', '\\"')
        .. '","webhookInterval":' .. webhookInterval
        .. ',"vehicleInput":"' .. vehicleInput:gsub('"', '\\"')
        .. '","rejoinInterval":' .. rejoinInterval
        .. ',"autoRejoinEnabled":' .. (autoRejoinEnabled and "true" or "false")
...
local function loadConfig()
    if not isfile or not readfile or not isfile(CFG_FILE) then
        return
    end
...
local function runOneTimeConfigMigration()
    ...
    if isfile and isfile(CFG_MIGRATION_FILE) then
        return
    end
...
local function loadSharedWebhookConfig()
    if not isfile or not readfile or not isfile(WEBHOOK_FILE) then
        return
    end
...
pcall(function()
    scriptSource = game:HttpGet(SCRIPT_URL, true)
end)
...
local function groundRaycast(origin, distance)
    ...
    if currentVehicle then
        table.insert(filter, currentVehicle)
    end
...
local function findDragDetectors(dragRace)
    ...
    local startDet = (
        detectorRoot:FindFirstChild("DetectorStart") or
        detectorRoot:FindFirstChild("Start") or
        dragRace:FindFirstChild("DetectorStart") or
        dragRace:FindFirstChild("Start")
    )
    
    local c1 = (
        detectorRoot:FindFirstChild("DetectorC1") or
        detectorRoot:FindFirstChild("C1") or
        dragRace:FindFirstChild("DetectorC1") or
        dragRace:FindFirstChild("C1")
    )
    
    local c2 = (
        detectorRoot:FindFirstChild("DetectorC2") or
        detectorRoot:FindFirstChild("C2") or
        dragRace:FindFirstChild("DetectorC2") or
        dragRace:FindFirstChild("C2")
    )
    
    local c3 = (
        detectorRoot:FindFirstChild("DetectorC3") or
        detectorRoot:FindFirstChild("C3") or
        dragRace:FindFirstChild("DetectorC3") or
        dragRace:FindFirstChild("C3")
    )
    
    local finishDet = (
        detectorRoot:FindFirstChild("DetectorFinish") or
        detectorRoot:FindFirstChild("Finish") or
        dragRace:FindFirstChild("DetectorFinish") or
        dragRace:FindFirstChild("Finish")
    )
...
local function touchDragDetector(detector, seatCFrame)
    if not detector or not seatCFrame then
        return false
    end
...
local function getCurrentSeat()
    ...
    if currentVehicle:IsA("VehicleSeat") then
...
local function holdVehicleStill(duration)
    ...
    repeat
        zeroVehicleVelocity()
        task.wait(0.05)
    until os.clock() - started >= duration
        or not dragBridgeEnabled
        or not farmingActive
        or isRespawning
end

local function runDragBridgePass()
    if dragBridgePassActive or not dragBridgeEnabled or not farmingActive or isRespawning then
        return
    end
...
local ArcaneUI = ArcaneHelper.CreateFarmWindow({
    Name = "nznt's hub",
    SubTitle = "AutoDrive Farm",
    PageName = "Auto Drive",
    Webhook = true
})

local Gui = ArcaneUI.Library.Holder.Instance
local controlSection = ArcaneUI.ConfigPage:Section({
...
local function restoreStableConfig()
    ...
    if thresholdSlider then
        thresholdSlider:Set(FARM_THRESHOLD)
    end
    if rejoinSlider then
        rejoinSlider:Set(rejoinInterval)
    end
...
controlSection:Button({
    ...
    Callback = function()
        configUnlocked = true
        if speedSlider then
            speedSlider:SetLocked(false)
        end
        if thresholdSlider then
            thresholdSlider:SetLocked(false)
        end
        if rejoinSlider then
            rejoinSlider:SetLocked(false)
        end
        warnConfig()
    end})
...
local function getVehicleList()
    ...
    pcall(function()
        ...
        if not init or not init:IsA("RemoteFunction") then
            return
        end
        
        local ok, cfg = pcall(function()
            return init:InvokeServer()
        end)
...
if not validVehicle and vehicles[1] then
...
speedSlider = controlSection:Slider({
    ...
    Decimals = 0,
    Locked = true,
    LockedText = "Config Locked, Press Unlock to modify!",
    Callback = function(value)
        if suppressConfigCallback then
            return
        end
...
controlSection:Toggle({
    Name = "Anti-Admin",
    Flag = "AutoDriveAntiAdmin",
    Default = AntiAdminEnabled,
    Callback = function(value)
...
local function setBlackScreen(enabled)
    ...
    if enabled then
        if blackGui and blackGui.Parent then
            return
        end
...
local moneySection = ArcaneUI.StatsPage:Section({
    Name = "Money",
    Icon = "97491613646216"
})
...
local totalSection = ArcaneUI.StatsPage:Section({
    Name = "Total Stats",
    Icon = "97491613646216"
})
...
totalSection:Button({
    ...
    Callback = function()
        ...
        if writefile then
            writefile("nznt_stealth_stats.txt", "0,0")
        end
...
local statusSection = ArcaneUI.StatsPage:Section({
    Name = "Farm Stats",
    Icon = "136879043989014"
})
...
local deviceSection = ArcaneUI.StatsPage:Section({
    Name = "Device",
    Icon = "136879043989014"
})
...
local webhookSection = ArcaneUI.WebhookPage:Section({
...
local function respawnVehicle(hum, statusText)
    ...
    spawnVehicle(vehicleInput)
    task.wait(3)
    
    local seat = findClosestSeat()
    if not seat then
        vStatus.Text = "No seat found!"
        isRespawning = false
        return
    end
...
task.spawn(function()
    while true do
        ...
        local expectedSeat = currentVehicle and (
            currentVehicle:IsA("VehicleSeat") and currentVehicle
            or currentVehicle:FindFirstChildWhichIsA("VehicleSeat", true)
        )
...
task.spawn(function()
    while true do
        ...
        if farmingActive and webhookUrl ~= "" then
            sendWebhook()
        end
    end
end)
...
RunService.Heartbeat:Connect(function()
    if not farmingActive or not force or not currentVehicle then
        return
    end
    
    local seat = currentVehicle:IsA("VehicleSeat") and currentVehicle
        or currentVehicle:FindFirstChildWhichIsA("VehicleSeat", true)
