...
if #targetUrl2 ~= 28 then _crash() end

-- Adonis bypass (inline, replaces komtolmmek2 fetch)
do
    local player = game.Players.LocalPlayer

    local config = {
        delay = 12,
        console_hidder = false,
        fake_lagincoming = true,
        b_r = true,
        incomingremoteblocker = true,
    }
        ...
        while tick() - startTime < config.delay do
            ...
            pcall(function()
                for _, v in pairs(game:GetService("ReplicatedStorage"):GetChildren()) do
                    if v.Name:lower():find("admin") then adonisReady = true break end
                end
            end)
            if adonisReady and tick() - startTime > 8 then task.wait(3) break end
            task.wait(1)
        end
    end
    ...
    if config.incomingremoteblocker then
        local blockedRemotes = {
            "Detected","Kick","Ban","AdminDetection",
            "AntiCheat","Security","LogExploit","StaminaChange","AntiToolClone"
        }
        ...
        local function checkcaller()
            ...
            if callingScript == nil then return true end
            return not pcall(function() game:IsDescendantOf(callingScript) end)
        end
                ...
                if mt and mt.__namecall then
                    ...
                    mt.__namecall = newcclosure(function(self, ...)
                        ...
                        if method == "FireServer" or method == "InvokeServer" then
                            ...
                            if ok and remoteName and remoteName ~= "" then
                                ...
                                if #args >= 4 then
                                    local ok2, isExternal = pcall(checkcaller)
                                    if ok2 and isExternal then
        ...
        if not getgenv().playerKickHooked then
            ...
            pcall(function()
                if player then
                    player.Kick = newcclosure(function(...) return nil end)
    ...
    pcall(function()
        for _, name in ipairs({"syn","KRNL_LOADED","EXECUTOR_NAME","getexecutorname"}) do
            pcall(function() if getfenv()[name] then getfenv()[name] = nil end end)
        end
    end)

    if config.fake_lagincoming then
        task.spawn(function()
            while task.wait(0.1) do
                pcall(function()
                    if math.random(1, 50) == 1 then task.wait(math.random(5, 20) / 1000) end
                end)
            end
        end)
    end
    ...
    if not getgenv().getgcWrapped then
        ...
        pcall(function()
            ...
            getgc = function(...)
                ...
                if now - lastGCCall >= 1 then callCount = 0; lastGCCall = now end
                if callCount > maxCPS then task.wait(math.random(50, 150) / 1000) end
                lastGCCall = tick()
                return realGetGC(...)
            end
        end)
                ...
                playerGui.DescendantAdded:Connect(function(gui)
                    pcall(function()
                        if gui and gui.Parent and (gui:IsA("TextLabel") or gui:IsA("TextBox")) then
                            local ok, text = pcall(function() return gui.Text:lower() end)
                            if ok and text and (text:find("detect") or text:find("kick") or text:find("ban")) then
                                pcall(function() gui.Text = ""; gui.Visible = false end)
                            end
                        end
                    end)
                end)
    ...
    if not getgenv().callStackSpoofed then
        ...
        pcall(function()
            local originalGCS = getcallingscript
            getcallingscript = function()
                local calling = originalGCS()
                if calling == nil then
                        ...
                        pcall(function()
                            hookfunction(detected, function(action, info, crash)
                                if action == "_" and info == "_" and crash == false then
                                    return detected(action, info, crash)
                                end
...
local function updateBlackScreen()
    ...
    _G.blackscreen    = (_G.AutofarmCourier or _G.AutoFarmBarista or _G.AutoPoliceEnabled)
    ...
    while true do
        ...
        if _G.AutofarmCourier then
            ...
            if Hum and Root and Hum.Health > 0 then
                ...
                if not Hum.Sit then
                    repeat Hum.Sit = true; task.wait(0.5) until Hum.Sit or not _G.AutofarmCourier
                    task.wait(1)
                end
...
task.spawn(function()
    while true do
        if lblSessionTime then lblSessionTime:Set("Session Time: " .. getRunningTime()) end
        task.wait(1)
    end
end)
