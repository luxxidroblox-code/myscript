...
-- ============================================
-- // STEP TELEPORT TRUCK (anti-detect)
-- ============================================
local function StepTeleport(model, targetCF, steps, interval)
    steps    = steps    or 20
    interval = interval or 0.15

    local startCF = model:GetPivot()
    for i = 1, steps do
        local stepCF = startCF:Lerp(targetCF, i / steps)
        model:PivotTo(stepCF)
...
-- // STEP TELEPORT PLAYER (anti-detect)
