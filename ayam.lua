warn("Projectsion - Loading Spoof Vehicle Method")
...
-- Range waktu teleport (sesuaikan dengan video)
local TP_MIN = 35
local TP_MAX = 39
...
-- ─── METODE TELEPORT (SPOOF VEHICLE DROP) ───────────
local function tweenTruckToDestination(truck, targetCF, duration, onTick)
    ...
    -- Tarik setinggi 3000 stud buat nahan posisi
    ...
    -- 1. HOLD DI LANGIT (Biar Waktu Terpenuhi)
    local elapsed = 0
    ...
    -- 2. SPOOF DROP (Suntik Velocity Palsu)
    if onTick then onTick(0, "dropping") end
    
    local steps = 8 -- Dibagi 8 frame turun biar ngerender pergerakan fisik
    for i = 1, steps do
        if not _G.Autofarm or not truck or not truck.Parent then break end
        
        -- Hitung posisi interpolasi ke bawah
        local currentLerpCF = skyCF:Lerp(targetLandingCF, i / steps)
        truck:PivotTo(currentLerpCF)
        
        -- Bikin Velocity palsu searah ke bawah menuju target
        local direction = (targetLandingCF.Position - truck:GetPivot().Position).Unit
        -- Anti NaN error
        if direction.X == direction.X then 
            local fakeSpeed = direction * 200 -- 200 adalah kecepatan spoof drop
            for _, part in ipairs(truck:GetDescendants()) do
                if part:IsA("BasePart") then
                    part.AssemblyLinearVelocity = fakeSpeed
                    part.AssemblyAngularVelocity = Vector3.zero
                end
            end
        end
        task.wait(0.05) -- Delay kecil buat sinkronisasi paket server
    end

    -- Pastikan mobil nyampe persis di titik
    if _G.Autofarm and truck and truck.Parent then
        truck:PivotTo(targetLandingCF)
        freezeVelocity()
    end

    -- Tahan di posisi bawah selama 2 detik biar trigger checkpoint/payment
    local dropEnd = os.clock() + 2
            ...
            while _G.Autofarm do
                ...
                if isTargetDestination(waypoint) then
                    ...
                    tweenTruckToDestination(myTruck, targetCFrame, tweenDuration, function(remaining, mode)
                        if DelayLabel then
                            local stateTxt = mode == "sky" and "Holding in Sky" or "Spoofing Drop (Claim)"
                            DelayLabel:Set({
...
local Window = Rayfield:CreateWindow({
    ...
    LoadingSubtitle  = "Version 4.6 (Spoof Vehicle Drop)",
