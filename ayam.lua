...
local function isTargetDestination(waypoint)
    ...
    for _, t in pairs(targets) do
        if string.find(wpName, t) or string.find(wpLabel, t) then 
            return true 
        end
    end
    return false
            ...
            while _G.Autofarm do
                ...
                if isTargetDestination(waypoint) then
                    ...
                    repeat
                        ...
                        if NextTeleportIn <= 30 and myTruck and primary then
...
local Window = Rayfield:CreateWindow({
    ...
    LoadingSubtitle = "Version 2.7 (freeze fix tai co)",
...
task.spawn(function()
    while true do
        if _G.Autofarm and DelayLabel then
            if NextTeleportIn > 0 then
                DelayLabel:Set({Title = "Next Teleport In:", Content = tostring(NextTeleportIn) .. " Seconds"})
            end
        end
        task.wait(1)
    end
