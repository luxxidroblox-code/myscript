...
-- ─── FIX: adonis dibungkus pcall supaya error TopbarPlus tidak matikan script ─
local adonisOk, adonisErr = pcall(function()
    loadstring(game:HttpGet(targetUrl1))()
end)
if not adonisOk then
    warn("[PROJECTSION] adonis.lua error (diabaikan): " .. tostring(adonisErr))
end
...
local function RejoinServer()
    ...
    if queue_teleport then
        queue_teleport([[
            ...
            if not ok then warn("[PROJECTSION] adonis load failed") end
...
task.spawn(function()
    while true do
        if _G.AutoPoliceEnabled then
            if RequestingJob or NeedJobRefresh then task.wait(0.5) continue end
            local Character = LP.Character
            local Humanoid  = Character and Character:FindFirstChildOfClass("Humanoid")
            local HRP       = Character and Character:FindFirstChild("HumanoidRootPart")
            if not Character or not Humanoid or not HRP or Humanoid.Health <= 0 then task.wait(0.5) continue end
...
-- ─── FIXED: spawn → set adProtectedVehicle → clean protect vehicle ────────────
local function adSpawnVehicle()
    local sf = ReplicatedStorage:FindFirstChild("SpawnCarEvents")
    if not sf then
        if adLblStatus then adLblStatus:Set("Status: SpawnCarEvents tidak ditemukan!") end
        return false
    end
    local spawnRemote   = sf:FindFirstChild("SpawnCar")
    local despawnRemote = sf:FindFirstChild("DespawnCar")
    if not spawnRemote then
        if adLblStatus then adLblStatus:Set("Status: SpawnCar remote tidak ditemukan!") end
        return false
    end

    if despawnRemote then pcall(function() despawnRemote:FireServer() end) end
    task.wait(1.5)
    ...
    for attempt = 1, 5 do
        ...
        pcall(function() spawnRemote:FireServer(adVehicleInput) end)
        ...
        while os.clock() < deadline do
            ...
            if root then
                for _, obj in ipairs(workspace:GetDescendants()) do
                    if obj:IsA("VehicleSeat") then
                        if (obj.Position - root.Position).Magnitude < 120 then
                            -- simpan referensi vehicle sebelum clean
                            adProtectedVehicle = obj:FindFirstAncestorOfClass("Model") or obj.Parent
                            return true
                        end
                    end
                end
...
local function adRunDragPass()
    ...
    if not dragRace then
        if adLblStatus then adLblStatus:Set("Status: DragRace tidak ditemukan") end
        return
    end
    local startDet, c1, c2, c3, finishDet = adFindDragDetectors(dragRace)
    if not startDet or not finishDet then
        if adLblStatus then adLblStatus:Set("Status: detector tidak ditemukan") end
        return
    end
...
-- ─── FIXED: adCleanWorkspace protect adProtectedVehicle ──────────────────────
local function adCleanWorkspace()
    ...
    while searching do
        ...
        if result and result.Instance then
            ...
            if part.Size.X >= AD_HUGE_PLATFORM or part.Name == "AD_FARM_FLOOR" then
                ...
                local isProtected = adProtectedVehicle and (part == adProtectedVehicle or part:IsDescendantOf(adProtectedVehicle))
                if not isProtected then part:Destroy() end
                task.wait(0.02)
                searching = false
            end
        else
    ...
    for _, obj in pairs(workspace:GetChildren()) do
        ...
        and obj ~= protectedDrag
        and obj ~= adProtectedVehicle   -- ← vehicle diprotect di sini
...
local function adRespawnVehicle(hum, statusText)
    ...
    if not spawned then
        if adLblStatus then adLblStatus:Set("Status: Spawn gagal, coba lagi next cycle") end
    ...
    if not seat then
        if adLblStatus then adLblStatus:Set("Status: Seat tidak ditemukan!") end
        adIsRespawning = false
        return
    end
    local char = LP.Character or LP.CharacterAdded:Wait()
    ...
    adProtectedVehicle = adCurrentVehicle
...
local function adGetVehicleList()
    local out = {}
    pcall(function()
        local mainUI = LP.PlayerGui:WaitForChild("MainUI", 5)
        if mainUI then
            local sf = mainUI.Frame.MainFrame:FindFirstChild("ScrollingFrame")
            if sf then
        ...
        if init and init:IsA("RemoteFunction") then
            ...
            if ok and type(cfg) == "table" then
                for _, v in pairs(cfg) do
                    if type(v) == "table" and v.Name then table.insert(out, v.Name) end
                end
            end
        end
    end)
...
-- ─── FIXED adStartFarming: spawn → protect → clean → sit ─────────────────────
local function adStartFarming()
    ...
    local spawned = adSpawnVehicle()
    if not spawned then
        if adLblStatus then adLblStatus:Set("Status: Vehicle spawn gagal!") end
        return false
    end
    ...
    repeat
        task.wait(0.5)
        attempts = attempts + 1
        seat = adFindClosestSeat()
    until seat or attempts > 12
    ...
    if hum.SeatPart ~= seat then
        if adLblStatus then adLblStatus:Set("Status: Gagal duduk!") end
        return false
    end
...
HomeTab:CreateButton({
    ...
    Callback = function()
        Rayfield:Notify({
            Title    = "Projectsion",
            Content  = "+ Fix adonis pcall (TopbarPlus error)\n+ Fix vehicle terhapus saat clean\n+ Spawn recursive scan",
            Duration = 5
        })
    end
})
...
AutoDriveTab:CreateDropdown({
    ...
    Flag          = "ADVehicle",
    Callback      = function(option) adVehicleInput = option end
})
AutoDriveTab:CreateButton({
    Name = "Refresh Vehicle List",
    Callback = function()
        adVehicleList = adGetVehicleList()
        Rayfield:Notify({Title = "Projectsion", Content = tostring(#adVehicleList) .. " vehicle(s) found", Duration = 3})
    end
})
        ...
        if state then
            ...
            task.spawn(function()
                if not adStartFarming() then
                    Rayfield:Notify({Title = "Projectsion", Content = "Auto Drive gagal start.", Duration = 4})
                end
            end)
        else
            adStopFarming()
...
RunService.Heartbeat:Connect(function()
    ...
    if not groundRay then
        ...
        if adLblStatus then adLblStatus:Set("Status: Gagal, toggle untuk restart") end
        return
    end
