...
-- vehicle yang sedang di-protect saat clean workspace
...
local function adFindClosestSeat()
    ...
    -- recursive scan supaya dapat vehicle nested
...
-- ─── FIXED: spawn → find seat → protect vehicle → clean → sit ────────────────
...
local function adSpawnVehicle()
    ...
    -- despawn dulu, kasih server settle
    ...
    for attempt = 1, 5 do
        ...
        while os.clock() < deadline do
            task.wait(0.3)
            if root then
                -- recursive: tangkap vehicle nested manapun
                for _, obj in ipairs(workspace:GetDescendants()) do
                    if obj:IsA("VehicleSeat") then
                        if (obj.Position - root.Position).Magnitude < 120 then
                            -- simpan model vehicle ke adProtectedVehicle sebelum return
...
local function adCleanWorkspace()
    ...
    while searching do
        ...
        if result and result.Instance then
            ...
            if part.Size.X >= AD_HUGE_PLATFORM or part.Name == "AD_FARM_FLOOR" then
                ...
                -- jangan hapus vehicle yang diprotect
                local isProtected = adProtectedVehicle and part:IsDescendantOf(adProtectedVehicle)
    ...
    for _, obj in pairs(workspace:GetChildren()) do
        ...
        -- PROTEKSI: jangan hapus vehicle yang baru spawn
        and obj ~= adProtectedVehicle
...
local function adRespawnVehicle(hum, statusText)
    ...
    -- spawn dulu, vehicle langsung diprotect di adSpawnVehicle
...
local function adStartFarming()
    ...
    -- 1. SPAWN DULU — adProtectedVehicle diset di dalam adSpawnVehicle
    ...
    -- 2. CLEAN — vehicle sudah diprotect, tidak akan terhapus
